...
const metadata = {
  name: "xh",
  fileExtension: "sh",
  identifier: "com.proxyman.plugin.xh",
  author: "Paw and Proxyman",
  version: "1.0.0",
};

// Include a template because we could not build require("fs") in webpack
// xh uses the same syntax as HTTPie but with 'xh' command instead of 'http'

const codeTemplate = `{{#body.has_multipart_body}}
# note: xh will post as form url-encoded if no file is specified
{{/body.has_multipart_body}}
{{! ----- }}
xh{{#body.has_form_encoded}} --form{{/body.has_form_encoded}}{{#body.has_json_encoded}} --json{{/body.has_json_encoded}} {{{request.method}}} '{{{request.url}}}' \\
{{! ----- }}
{{#headers.has_headers}}
{{#headers.header_list}}
